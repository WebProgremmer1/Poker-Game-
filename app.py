...
custom_theme = gr.themes.Soft(primary_hue="green").set(color_accent="#39FF14")
...
def make_card_image(
    ...
    ax.text(0, 0.05, label, ha="center", va="center", fontsize=15, color=text_color, fontweight="bold")
...
def get_card_tile(label: str, active: bool):
    ...
    if active:
        img = make_card_image(label, edge_color=NEON_RED, text_color=NEON_RED, fill_color=CARD_BG_ACTIVE, glow=True, glow_width=12, dpi=150)
    else:
        img = make_card_image(label, edge_color=NEON_GREEN, text_color=NEON_GREEN, fill_color=CARD_BG, glow=True, glow_width=10, dpi=150)
...
def random_cards(num_cards):
    ...
    for _ in range(num_cards):
        while True:
            ...
            suits = random.choice(lst_suits)
            cards_suits = f"{card_num}{suits}"
...
def compose_strip(labels, from_idx, to_idx, t, active_indices):
    if active_indices is None:
        active_indices = set()
    tiles = [get_card_tile(lbl, active=(i in active_indices or i == from_idx)) for i, lbl in enumerate(labels)]
    w, h = tiles[0].width, tiles[0].height
    gap_x = int(w * 0.14)
    gap_y = int(h * 0.10)
    anchors, total_w, total_h = _grid_anchors(len(tiles), w, h, gap_x, gap_y)
...
def animate(num_cards: str, delay_ms: int, frames_per_move: int = 10):
    ...
    if n == 0:
        yield make_card_image("Enter 1–52", edge_color=NEON_GREEN, text_color=NEON_GREEN, fill_color=CARD_BG, glow=True, glow_width=10)
        return
